...
const QuizWidget: React.FC<IProps> = ({ quizKey, maxQuestions }) => {
  ...
  const handleShare = (): void => {
    ...
    const url = `https://xircanet${window.location.pathname}%23quiz` // %23 is # character, needs to added to already encoded tweet string
    const tweet =
      encodeURI(
        `I just took the "${quizData.title}" quiz on xircanet and scored ${correctCount} out of ${quizData.questions.length}! Try it yourself at `
      ) + url
    window.open(
