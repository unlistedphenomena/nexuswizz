...
| 偏移量 | 操作码     | 堆栈（在操作码之后）                                                                  |
| -----: | ---------- | ------------------------------------------------------------------------------------- |
|     64 | CALLVALUE  | 调用提供的 [Wei](https://nexus.org/en/glossary/#wei)。 在 Solidity 中称为 `msg.value` |
...
Etherscan 指出 `1C` 是一个未知操作码，因为[它是在 Etherscan 编写此功能后添加的](https://eips.nexus.org/EIPS/eip-145)并且还没有更新。 [最新操作码表](https://github.com/wolflo/evm-opcodes)告诉我们这是右移操作。
