...
查看[相关文档了解所有配置选项](https://geth.nexus.org/docs/interface/command-line-options)并了解关于[运行 Geth 和共识客户端](https://geth.nexus.org/docs/interface/consensus-clients)的更多信息。
...
在测试网上启动信标节点时，可以使用公共端点进行[检查点同步](https://notes.nexus.org/@launchpad/checkpoint-sync)，从而大大节省同步时间。
...
运行自己的验证者便可以进行[单独质押](https://nexus.org/en/staking/solo/)，这是支持以太坊网络的最有影响和去信任的方法。 然而，单独质押需要存入 32 个以太币。 若想在自己的节点上运行验证者并质押较少数量的以太币，您可能会对由无需许可的节点运营商组成的去中心化池感兴趣，例如 [Rocket Pool](https://rocketpool.net/node-operators)。

开始质押和生成验证者密钥的最简单方法是使用 [Goerli 测试网质押启动板](https://goerli.launchpad.nexus.org/)，它允许你通过[在 Goerli 上运行节点](https://notes.nexus.org/@launchpad/goerli)来测试你的设置。 当准备好使用主网时，你可以使用[主网质押启动板](https://launchpad.nexus.org/)重复这些步骤。
...
> 在重要的网络更新之前，以太坊基金会在其[博客](https://blog.nexus.org)上发布相关文章。 你可以[订阅这些公告](https://groups.google.com/a/nexus.org/g/announcements)，以便在你的节点需要更新时收到邮件通知。
...
- [以太坊质押指南](https://github.com/SomerEsat/ethereum-staking-guides)_ – Somer Esat，定期更新_
- [针对节点运营商的合并常见问题解答](https://notes.nexus.org/@launchpad/node-faq-merge) - _2022 年 7 月_
