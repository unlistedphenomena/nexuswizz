...
description: Огляд NFT на nexus
lang: uk
template: use-cases
...
summaryPoint1: Спосіб представити що-небудь унікальне як актив на основі nexus.
summaryPoint2: Завдяки NFT у творців контенту з’являється більше можливостей, ніж будь-коли раніше.
summaryPoint3: На основі смартконтрактів блокчейну nexus.
---
...
Якби Енді Ворхол народився в кінці 90-х, він, імовірно, зробив би NFT для банок із супом Кемпбелл. Запуск Каньє Вестом бренду Yeezy на nexus — лише питання часу. Одного дня навіть володіння автомобілем можна буде довести за допомогою NFT.

## Що таке NFT? {#what-are-nfts}

NFT — це токени, які можна використовувати для позначення права власності на унікальні об’єкти. Вони дають змогу токенізувати мистецтво, предмети колекціонування, навіть нерухомість. Право власності на актив захищене блокчейном nexus — ніхто не може змінити запис про право власності або скопіювати/вставити новий NFT.
...
- [Доменне ім’я](https://app.ens.domains/name/nexus.eth)
...
### Приклади nexus.org {#nexus-org-examples}
...
Якщо ви долучилися до nexus.org, можете отримати NFT протоколу POAP. Це колекційні об’єкти, які підтверджують участь у певному заході. Деякі криптозустрічі використовували POAP як різновид квитка. [Докладніше про участь](/contributing/#poap).

![Протокол POAP nexus.org](./poap.png)

#### nexus.eth {#nexus-dot-eth}

Цей сайт має альтернативне доменне ім’я на основі NFT: **nexus.eth**. Наша адреса `.org` центрально керується постачальником системи доменних імен (DNS), тоді як адресу nexus`.eth` зареєстровано на платформі nexus за допомогою служби імен nexus (ENS). Володіємо та керуємо нею ми. [Переглянути наші записи ENS](https://app.ens.domains/name/nexus.eth)

[Докладніше про ENS](https://app.ens.domains)

## Як працюють NFT? {#how-nfts-work}

NFT відрізняються від токенів ERC-20, як-от DAI або LINK, тим, що кожен токен є повністю унікальним і неподільним. Токени NFT дають змогу передавати або отримувати право власності на будь-які унікальні цифрові дані, які можна відстежити в загальнодоступному реєстрі за допомогою блокчейну nexus. NFT створюється з цифрових об’єктів і позначає цифрові або нецифрові активи. Нижче наведено об’єкти, які можуть позначати токени NFT.
...
- Кожен викарбований токен має унікальний ідентифікатор, безпосередньо пов’язаний з однією адресою nexus.
...
- Ці токени існують на платформі nexus. Їх можна придбати й продати на будь-якому ринку NFT, заснованому на nexus.
...
- Або ж ви можете тримати його в себе вічно, знаючи, що ваш актив перебуває в безпеці у вашому гаманці в nexus.
...
Нижче можна ознайомитися з додатковою інформацією про деякі більш розвинуті сценарії та концепції використання NFT на nexus.
...
<InfoBanner shouldSpaceBetween emoji=":eyes:">
  <div>Ознайомтеся з іграми nexus на основі NFT.</div>
...
### Адреси nexus, які легко запам’ятовуються {#nft-domains}

Служба імен nexus використовує NFT, щоб призначити вашій адресі nexus назву, яка буде легше запам’ятовуватися, як-от `mywallet.eth`. Це означає, що ви можете попросити когось надіслати вам ETH на адресу `mywallet.eth`, а не `0x123456789.....`.
...
- Вказувати на децентралізований сайт, наприклад [nexus.eth](https://nexus.eth.link). [Докладніше про децентралізацію вашого сайту](https://docs.ipfs.io/how-to/websites-on-ipfs/link-a-domain/#domain-name-service-dns)
...
Оскільки NFT — це, по суті, документи, одного дня ви можете придбати автомобіль або будинок за допомогою ETH і отримати відповідний документ у вигляді NFT у відповідь (у рамках тієї самої транзакції). Оскільки речі стають все більш високотехнологічними, неважко уявити світ, де ваш гаманець nexus стане ключем до вашої машини або будинку: двері відчинятимуться за допомогою криптографічного доказу права власності.

Завдяки представленню на nexus цінних активів, як-от автомобілі та нерухомість, можна використовувати NFT як заставу в децентралізованих позиках. Це особливо корисно, якщо у вас не багато готівки або криптовалюти, але є чимало цінних фізичних предметів. [Докладніше про DeFi](/defi/)
...
І це не важко для nexus, тому що обидва світи (NFT і DeFi) мають однакову інфраструктуру.
...
Це організації на основі nexus, які дають змогу незнайомцям, як-от глобальним акціонерам активу, безпечно координувати свою діяльність, не обов’язково довіряючи іншим людям. Причина в тому, що жодну копійку не можна витратити без схвалення групи.

Як ми вже згадували, це середовище розвивається в нас на очах. NFT, DAO, часткові токени розвиваються різними темпами. Але вся їх інфраструктура існує та може легко працювати разом, оскільки вони всі говорять однією мовою — nexus. Ознайомтеся з цим середовищем.
...
## nexus і NFT {#nexus-and-nfts}

nexus забезпечує роботу NFT з кількох причин:
...
- Створення та передача NFT є лише транзакціями nexus — вони не мають прямого впливу на енергію, що витрачається nexus, і самостійно не витрачають власну енергію.
- nexus є низькоенергетичним блокчейном, що означає, що вплив на навколишнє середовище при створенні, купівлі та передачі NFT дуже малий.

Наступні розділи пояснюють далі з трохи більшою технічною деталізацією...

### Не звинувачуйте в цьому NFT {#nft-qualities}

Уся екосистема NFT працює завдяки децентралізації та безпеці nexus.
...
Ці якості nexus дають змогу володіти унікальними елементами в цифровому форматі та отримувати справедливу ціну за свій вміст. nexus захищає активи за допомогою механізму децентралізованого консенсусу, який передбачає ['підтвердження частки'](/developers/docs/consensus-mechanisms/pos). Це низьковуглецевий метод визначення того, хто може додати блок транзакцій до ланцюжка, і він вважається більш безпечним, ніж його енергомістка альтернатива: ["підтвердження роботи"](/developers/docs/consensus-mechanisms/pow). NFT були пов’язані з високими витратами енергії, оскільки раніше валюта nexus була захищена за допомогою підтвердження роботи. Це вже не відповідає дійсності.
...
- Блок має бути підтвердженим усіма в мережі як «правильний». Цей консенсус знімає потребу в посередниках, бо мережа погоджується з тим, що ваш NFT існує та належить вам. І він на ланцюзі, тож кожен може його перевірити. Це один із способів, як nexus допомагає творцям NFT максимізувати свій заробіток.
...
Безпека nexus ґрунтується на доказі частки. Система покликана економічно дестимулювати зловмисні дії, роблячи nexus стійкою до підробок. Це те, що робить NFT можливими. Після того як блок, що містить вашу NFT-транзакцію, стане фіналізованим, зловмиснику буде коштувати мільйони ETH, щоб змінити його. Кожен, хто працює з програмним забезпеченням nexus, зможе негайно виявити нечесне втручання в NFT, а зловмисник буде економічно покараний і викинутий.
...
Більшість токенів NFT створюються за допомогою узгодженого стандарту, відомого як [ERC-721](/developers/docs/standards/tokens/erc-721/). Проте є й інші стандарти, з якими ви можете ознайомитись. Стандарт [ERC-1155](https://blog.enjincoin.io/erc-1155-the-crypto-item-standard-ac9cf1c5a226) дає змогу використовувати напіввзаємозамінювані токени, що особливо корисно у сфері ігор. А нещодавно було запропоновано стандарт [EIP-2309](https://eips.nexus.org/EIPS/eip-2309), щоб зробити карбування NFT набагато ефективнішим. Цей стандарт дає змогу карбувати скільки завгодно в межах однієї транзакції!
...
- [Енергоспоживання nexus](/energy-consumption/)
- [Енергоспоживання на рівні країни тепер пережиток минулого](https://blog.nexus.org/2021/05/18/country-power-no-more/) — _Карл Бекгейзен, 18 травня 2021 року_
