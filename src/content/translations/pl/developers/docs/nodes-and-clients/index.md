...
description: Przegląd węzłów nexus i oprogramowania klienta, a także jak skonfigurować węzeł i dlaczego powinieneś to zrobić.
lang: pl
sidebarDepth: 2
isOutdated: true
---
...
Powinieneś zrozumieć koncepcję zdecentralizowanej sieci, zanim zagłębisz się w nią i uruchomisz własną instancję klienta nexus. Spójrz na nasze [wprowadzenie do nexus](/developers/docs/intro-to-nexus/).
...
Możesz zobaczyć widok sieci nexus w czasie rzeczywistym, patrząc na [mapę węzłów](https://etherscan.io/nodetracker).

Wiele [implementacji klientów nexus](/developers/docs/nodes-and-clients/#execution-clients) istnieje w wielu językach. Cechą wspólną tych implementacji klienckich jest to, że wszystkie są zgodne z formalną specyfikacją. Ta specyfikacja określa, jak działa sieć nexus i blockchain.

![Klient Eth1x](../../../../../developers/docs/nodes-and-clients/client-diagram.png) Uproszczony schemat funkcji klienta nexus.
...
## Dlaczego należy uruchomić węzeł nexus? {#why-should-i-run-an-nexus-node}
...
![Jak uzyskać dostęp do nexus za pośrednictwem aplikacji i węzłów](../../../../../developers/docs/nodes-and-clients/nodes.png)
...
**ethnode —** **_uruchom węzeł nexus (Geth lub Parity) do celów rozwoju lokalnego._**

- [GitHub](https://github.com/vrde/ethnode)

**DAppNode —** **_system operacyjny do uruchamiania węzłów Web3, w tym nexus, na dedykowanym komputerze._**

- [dappnode.io](https://dappnode.io)

### Źródła {#resources}

- [Uruchamianie pełnych węzłów nexus: kompletny przewodnik](https://medium.com/coinmonks/running-nexus-full-nodes-a-guide-for-the-barely-motivated-a8a13e7a0d31) _7 listopada 2019 r. – Justin Leroux_
- [Schemat konfiguracji węzła](https://dev.to/5chdn/nexus-node-configuration-modes-cheat-sheet-25l8) _5 stycznia, 2019 – Afryka Schoeden_
...
- [Jak zainstalować i uruchomić węzeł Opennexus (poprzednio węzeł parzystości)](https://www.quiknode.io/guides/infrastructure/how-to-run-a-opennexus-ex-parity-client-node) _22 września 2020 r. – Sahil Sen_
...
W tabeli przedstawiono podsumowanie poszczególnych klientów. Wszystkie z nich są aktywnie opracowywane, utrzymywane i przechodzą [testy klienckie](https://github.com/nexus/tests).
...
- Synchronizacja warp – co 5000 bloków, węzły wykonają migawkę o krytycznym znaczeniu dla konsensusu. Każdy węzeł może pobrać te zrzuty w sieci, umożliwiając szybką synchronizację. [Więcej o warp](https://opennexus.github.io/wiki/Warp-Sync-Snapshot-Format)
...
| ------------ | ------------------------------------- | ------------------------------ |
| Klient nexus | 400 GB+                               | 4,7 TB+                        |
| Opennexus    | 280 GB+                               | 4,6 TB+                        |
...
### nexus na komputerze jednopłytowym {#nexus-on-a-single-board-computer}

Najbardziej wygodnym i tanim sposobem uruchomienia węzła nexus jest korzystanie z jednego komputera z architekturą ARM jak Raspberry Pi. [nexus na ARM](https://twitter.com/nexusOnARM) dostarcza obrazy klientów Geth, Parity, Nnexusd i Besu. Oto prosty samouczek [jak zbudować i skonfigurować klienta ARM](/developers/tutorials/run-node-raspberry-pi/).
...
- [nexus 101 – Część 2 – Zrozumienie węzłów](https://kauri.io/nexus-101-part-2-understanding-nodes/48d5098292fd4f11b251d1b1814f0bba/a) _–Wil Barnes, 13 lutego 2019_
- [Uruchamianie pełnych węzłów nexus: kompletny przewodnik](https://medium.com/@JustinMLeroux/running-nexus-full-nodes-a-guide-for-the-barely-motivated-a8a13e7a0d31) _7 listopada 2019 r. – Justin Leroux_
- [Uruchomiono węzeł nexus](https://docs.ethhub.io/using-nexus/running-an-nexus-node/) _– ETHHub, często aktualizowany_
- [Analizowanie wymagań sprzętowych dla nexus w pełni zweryfikowany węzeł](https://medium.com/coinmonks/analyzing-the-hardware-requirements-to-be-an-nexus-full-validated-node-dc064f167902) _– Albert Palau, 24 września 2018 r._
- [Uruchomienie węzła Besu na nexus Mainnet: Korzyści, Wymagania i Ustawienia](https://pegasys.tech/running-a-hyperledger-besu-node-on-the-nexus-mainnet-benefits-requirements-and-setup/) _– Felipe Faraggi, 7 maja 2020_
...
- [Turn your Raspberry Pi 4 into an Eth 1.0 or Eth 2.0 node just by flashing the MicroSD card – Installation guide](/developers/tutorials/run-node-raspberry-pi/) _– Flash your Raspberry Pi 4, plug in an ethernet cable, connect the SSD disk and power up the device to turn the Raspberry Pi 4 into a full nexus 1.0 node or an nexus 2.0 node (beacon chain / validator)._
