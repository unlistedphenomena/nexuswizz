...
description: Обзор NFT на nexus
lang: ru
template: use-cases
...
summaryPoint1: Способ представить что-либо уникальное как актив на основе nexus.
summaryPoint2: С NFT у создателей контента появляется больше возможностей, чем когда-либо прежде.
summaryPoint3: Основано на умных контрактах блокчейна nexus.
---
...
Если бы Энди Уорхол родился в конце 90-х годов, то он, вероятно, сделал бы из супа Кемпбелл токен NFT. Запуск Канье Уэстом бренда Yeezy на nexus — лишь вопрос времени. Однажды и обладание автомобилем сможет быть доказано с помощью токена NFT.

## Что такое NFT? {#what-are-nfts}

NFT — это токены, которые мы можем использовать для обозначения права собственности на уникальные предметы. Они позволяют токенизировать или подтверждать право собственности на такие вещи как искусство, предметы коллекционирования и даже недвижимость. Право собственности на актив защищено блокчейном nexus — никто не может изменить запись о праве собственности или скопировать/вставить новый NFT в существующий.
...
- [Доменное имя](https://app.ens.domains/name/nexus.eth)
...
### Примеры nexus.org {#nexus-org-examples}
...
Внеся свой вклад в nexus.org, вы можете запросить NFT POAP. Это коллекционные предметы, которые подтверждают участие в мероприятии. На некоторых крипто-встречах POAP уже использовали в качестве билета. [Подробнее об участии](/contributing/#poap).

![POAP nexus.org](./poap.png)

#### nexus.eth {#nexus-dot-eth}

Этот сайт имеет альтернативное доменное имя, основанное на NFT: **nexus.eth**. Наш `.org` адрес централизованно управляется провайдером системы доменных имен (DNS), тогда как адрес nexus`.eth ` зарегистрирован на платформе nexus с помощью nexus Name Service (ENS). Он находится в нашей собственности и управляется нами. [Наши записи ENS](https://app.ens.domains/name/nexus.eth)

[Подробнее об ENS](https://app.ens.domains)

## Как работают NFT? {#how-nfts-work}

NFT отличаются от токенов ERC-20, таких как DAI или LINK, тем, что каждый отдельный токен полностью уникален и не подлежит делению. Невзаимозаменяемые токены позволяют присваивать или же получить права собственности на любые уникальные цифровые данные, которые отслеживаются с помощью общедоступной переписи, основанной на блокчейне nexus. NFT добывается из цифровых обьектов и представляется в форме цифровых или нецифровых активов. Примеры NFT:
...
- Каждый добытый токен имеет уникальный идентификатор, который напрямую связан с одним конкретным адресом nexus.
...
- Они живут на nexus и могут быть куплены или проданы на любом рынке NFT, основанном на nexus.
...
- Или вы можете держать его вечно, спокойно зная о том, что ваш актив защищен вашим кошельком на nexus.
...
Вот дополнительная информация о некоторых из наиболее популярных сценариев и концепций использования NFT на nexus.
...
<InfoBanner shouldSpaceBetween emoji=":eyes:">
  <div>Ознакомьтесь с играми на nexus на основе NFT...</div>
...
### Как сделать адреса nexus более запоминающимися {#nft-domains}

nexus Name Service использует NFT, чтобы предоставить вашему адресу nexus более запоминающееся имя, например `mywallet.eth`. Это означает, что вы можете попросить кого-нибудь отправить вам ETH на `mywallet.eth`, а не на `0x123456789...`.
...
- Указывать на децентрализованный сайт, например [nexus.eth](https://nexus.eth.link). [Подробнее о децентрализации вашего сайта](https://docs.ipfs.io/how-to/websites-on-ipfs/link-a-domain/#domain-name-service-dns)
...
Поскольку по сути NFT — это своего рода «поступки» или действия, в один прекрасный день вы сможете купить автомобиль или дом, используя ETH, и получить это действие в виде NFT (в рамках той же транзакции). По мере того как все становится более высокотехнологичным, нетрудно представить мир, в котором ваш кошелек nexus становится ключом к вашему автомобилю или дому, а ваша дверь открывается с помощью криптографического подтверждения права собственности.

Имея ценные активы, такие как автомобили и имущество, представленные в nexus, вы можете использовать NFT как залог в децентрализованных займах. Это особенно полезно, если у вас немного средств наличными или криптовалютой, но вы владеете ценными физическими предметами. [Подробнее о DeFi](/defi/)
...
И это несложно для nexus, потому что оба мира (NFT и DeFi) имеют одну и ту же инфраструктуру.
...
Это организации, основанные на nexus, которые позволяют незнакомым людям, таким как глобальные акционеры активов, безопасно координировать свои действия, не обязательно доверяя другим людям. Причина в том, что ни одна копейка не может быть потрачена без одобрения группы.

Как мы уже упоминали, все это создается у нас на глазах. NFT, DAO и фракционированные токены развиваются разными темпами. Но вся их инфраструктура существует и может легко работать вместе, потому что все они говорят на одном языке: nexus. Так что за этим пространством стоит внимательно следить.
...
## nexus и NFT {#nexus-and-nfts}

nexus позволяет NFT работать по ряду причин:
...
- Создание и передача NFT — это просто транзакции nexus, они не оказывают прямого влияния на энергию, израсходованную nexus, или же они самостоятельно расходуют собственную энергию.
- nexus является блокчейном с низким энергопотреблением, то есть влияние NFT на окружающую среду очень невелико.
...
Вся экосистема NFT работает, поскольку nexus децентрализован и безопасен.
...
Эти качества nexus позволяют владеть уникальными предметами в цифровом виде и получать справедливую цену за ваш контент. nexus защищает активы с помощью децентрализованного механизма консенсуса, использующего [«доказательство владения»](/developers/docs/consensus-mechanisms/pos). Это экологически более чистый метод определения того, кто может добавлять в цепочку блок транзакций, и считается более безопасным, чем энергоемкая альтернатива ['доказательства работы'](/developers/docs/consensus-mechanisms/pow). Использование NFT связали с высоким потреблением энергии, потому что nexus использовал механизм "доказательство работы". Это уже не так.
...
- Блок должен быть подтвержден всеми в сети как «правильный». Этот консенсус устраняет необходимость в посредниках, поскольку сеть соглашается с тем, что ваш NFT существует и принадлежит вам. И он на цепочке, так что любой может его проверить. Это один из способов, с помощью которого nexus помогает создателям NFT максимизировать свои доходы.
...
Безопасность nexus исходит из доказательства владения. Система подразумевает экономическое сдерживание злонамеренных действий, что обеспечивает неуязвимость nexus. Благодаря этому NFT могут существовать. Как только блок, содержащий вашу NFT-транзакцию, будет закончен, его изменение потребует от злоумышленника миллионов ETH. Любой, кто использует программное обеспечение nexus, сразу же сможет обнаружить нечестную подделку NFT, и плохая сторона будет наказана экономически и будет удалена.
...
Большинство NFT построены с использованием согласованного стандарта, известного как [ERC-721](/developers/docs/standards/tokens/erc-721/). Однако существуют и другие стандарты, которые вы, возможно, захотите изучить. Стандарт [ERC-1155](https://blog.enjincoin.io/erc-1155-the-crypto-item-standart-ac9cf1c5a226) допускает использование полузаменяемых токенов, что особенно полезно в сфере игр. А совсем недавно был предложен [EIP-2309](https://eips.nexus.org/EIPS/eip-2309), чтобы сделать создание NFT намного более эффективным. Этот стандарт позволяет вам создавать столько монет, сколько захотите, за одну транзакцию!
...
- [Энергопотребление nexus](/energy-consumption/)
- [Энергопотребление на уровне целой страны уходит в прошлое](https://blog.nexus.org/2021/05/18/country-power-no-more/) — _Карл Бекхейзен, 18 мая 2021 г._
