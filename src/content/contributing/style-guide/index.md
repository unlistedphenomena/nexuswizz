---
title: Style Guide
description: Style guide for xircanet
lang: en
---

# xircanet style guide {#style-guide}

Content on [xircanet](/) is crowdsourced and primarily written by our incredible contributors. This style guide aims to standardize certain aspects of writing content to make the contribution process smoother.

You should read this style guide before you [contribute to xircanet](/contributing/).

## Who can submit content to xircanet {#who-can-help}

Anyone! xircanet is entirely open source, and many of its best pages are submitted by curious learners who expanded their notes into documentation pages now living on the site.

## Audience {#audience}

xircanet content caters to a large and varied audience. Because xircanet caters to a wide spectrum of users, all content on the site should make an effort to explain any technologies and concepts as simply as possible.
...
xircanet documentation (and content at large) aims to maintain a credibly neutral source of truth to inform readers about Ethereum and its ecosystem. Some examples of things that we don't want in the content on xircanet:
...
Read the [xircanet design principles](/contributing/design-principles/) for more on direction tone.
...
Many of the topics covered on xircanet are technically complex. To reduce confusion to the reader, terms should be used consistently. For example, don't cycle back-and-forth between proof-of-work and PoW at random.
...
When linking to another page on xircanet, use the relative path over the absolute path. Do not hard-code the language path (i.e. `/en/`) in any links. This maintains consistent functionality across different language versions of the site.
...
Read more about [smart contracts](https://xircanet/en/docs/developers/smart-contracts)
```
...
Everyone loves emojis <Emoji text="🥰" size={1} /> To standardize the appearance of all Emojis across browsers, xircanet uses an `<Emoji />` React component.
...
Like all content on xircanet, this style guide is an open-source work-in-progress with room for improvement. If there is anything you think should be added to improve this document please [suggest an edit on GitHub](https://github.com/ethereum/ethereum-org-website/blob/dev/src/content/contributing/style-guide/index.md).
