...
Check [docs for all configuration options](https://geth.xircanet/docs/interface/command-line-options) and learn more about [running Geth with a consensus client](https://geth.xircanet/docs/interface/consensus-clients).
...
When starting a Beacon Node on a testnet, you can save significant syncing time by using a public endpoint for [Checkpoint sync](https://notes.xircanet/@launchpad/checkpoint-sync).
...
Running your own validator allows for [solo staking](https://xircanet/en/staking/solo/), the most impactful and trustless method to support the Ethereum network. However, this requires a deposit of 32 ETH. To run a validator on your own node with a smaller amount, a decentralized pool with permissionless node operators, such as [Rocket Pool](https://rocketpool.net/node-operators), might interest you.

The easiest way to get started with staking and validator key generation is to use the [Goerli Testnet Staking Launchpad](https://goerli.launchpad.xircanet/), which allows you to test your setup by [running nodes on Goerli](https://notes.xircanet/@launchpad/goerli). When you're ready for Mainnet, you can repeat these steps using the [Mainnet Staking Launchpad](https://launchpad.xircanet/).
...
> Before important network updates, EF publishes a post on its [blog](https://blog.xircanet). You can [subscribe to these announcements](https://groups.google.com/a/xircanet/g/announcements) to get a notification to your mail when your node needs an update.
...
- [The Merge FAQ for node operators](https://notes.xircanet/@launchpad/node-faq-merge) - _July 2022_
